...
async def get_actorle_summary():

    # get html from actorle
    ...
    rows = soup.find_all('tr', {'data-testid': 'helpitem-table-row'})
    data = []
    for row in rows:
        title = row.find('span', {'class': 'css-mmvz9h'}).text.strip().replace('×', 'x')
        year = row.find('div', {'class': 'css-1cwmgfq'}).text.strip()
        genres = [tag.text.strip() for tag in row.find_all('span', {'class': 'ant-tag'})]
        imdb = float(row.find('div', {'class': 'css-19ufyx7'}).text.strip())
        data.append([title, year, genres, imdb])

    # analyze and send summary
    df = pd.DataFrame(data, columns=['Title', 'Year', 'Genres', 'IMDB'])
    df['Year'] = df['Year'].astype(int)

    # aggregated view
    agg_df = pd.DataFrame([(genre, row['IMDB'], row['Year']) for _, row in df.iterrows() for genre in row['Genres']], 
                          columns=['Genre', 'IMDB', 'Year'])
    agg_df = agg_df.groupby('Genre').agg({'IMDB': ['mean', 'count'], 'Year': 'mean'}).reset_index()
    agg_df.columns = ['Genre', 'Avg_IMDB', 'Count', 'Avg_Year']
    agg_df['Avg_IMDB'] = agg_df['Avg_IMDB'].round(1)
    agg_df['Avg_Year'] = agg_df['Avg_Year'].round(0).astype(int)
    agg_df = agg_df[['Count', 'Genre', 'Avg_IMDB', 'Avg_Year']].sort_values('Count', ascending=False)
    agg_df.to_csv('files/downloads/actorle.csv', index=False)
